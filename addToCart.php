...
if (isset($_POST['idProduct']) && isset($_POST['quantity']) && isset($_POST['mailCustomer'])) {
    ...
    if (count($result) > 0) {
        ...
        $sql = "INSERT INTO `CartItem` (`idCart`,`quantity`, `idProduct`) VALUES (:idCart , :quantity, :idProduct)";
        $query = $con->prepare($sql);
        $query->execute(['idCart' => $idCart, 'quantity' => $_POST['quantity'], 'idProduct' => $_POST['idProduct']]);
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $message = array(
            "message" => "Product added to cart"
        );
